...
public abstract class Functions {
	...
	public static boolean isValidIP(String t) {
		String[] all = removeBlanks(t).split(".");
		StringBuilder ip = new StringBuilder("");
		for(int i = 0 ; i < all.length ; ++i)
			for(int j = 0 ; j < all[i].length() ; ++j)
				if(Character.isDigit(all[i].charAt(j)) || all[i].charAt(j) == '.')
					ip.append(all[i].charAt(j));
				else return false;
		try {
			ClientMain.ip = InetAddress.getByName(ip.toString());
	...
	public static boolean initClient() throws IOException {
		...
		if(t == -1 || t > 50) {
			...
			return false;
		} else ClientMain.window = (byte)t;
		while((s = in.readLine()) != null)
			if(!removeBlanks(s).isEmpty())
				break;
		try {
			double p = Double.parseDouble(s);
			ClientMain.probability = p;
		} catch(Exception e) {
	...
	public static boolean initServer() throws IOException {
		...
		if(t == -1 || t > 50) {
			...
			System.out.println(t);
			return false;
		} else ServerMain.window = (byte)t;
		while((s = in.readLine()) != null)
			if(!removeBlanks(s).isEmpty())
				break;
		try {
			double p = Double.parseDouble(s);
			ServerMain.probability = p;
		} catch(Exception e) {
			System.out.println("\nInvalid or missing probability\n\n");
			return false;
		}
